...
| `ldown` | 2 | 目标位置向上仰视 | 下行天空辐亮度（仅 `thermal` 波段：mwir/lwir；`sky` 路径不跑——其 lpath 即穹顶辐亮度） |
...
| `view_zenith_deg` | 观测天顶角 [°]；slant 采样 (90,180]（向下），sky 采样 [0,90)（向上），horizontal 恒 90 | [0, 180] | 180 |
...
| `nir` | 8333–10753 | 0.93–1.2 µm | 2421 | 否 |
| `swir` | 4167–7143 | 1.4–2.4 µm | 2977 | 否（300 K 热发射在 2.4 µm 可忽略） |
...
- **sky**（望天/穹顶）：采样自由度 `(h1_km, view_zenith_deg)`，天顶角 **[0, 90°) 向上**，
  H2≡100 km（TOA）。该路径的 **lpath 就是 (方向, 天气) 的天空穹顶辐亮度**——供下游
  miss 光线逐方向查询；tau 是到太空的透过率（太阳方向即直射衰减）。**不跑 ldown**
  （穹顶数据集是它的上位替代）。T/RH/P 覆盖写成 [0, min(h1, 1 km)] 等温面层：低空
  观察者的上行路径从天气层内起步，高空观察者从模式大气起步。range_km 非自由度（恒 0）。
